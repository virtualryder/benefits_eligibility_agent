#!/usr/bin/env bash
# prove_connector.sh — prove the governed OAuth connector end to end. Usage: bash connector/prove_connector.sh <agent_dir>
set -uo pipefail
export AWS_PAGER=""
AGENT_DIR="${1:?usage: prove_connector.sh <agent_dir>}"
SELF="$(cd "$(dirname "$0")" && pwd)"; LIB="$(cd "$SELF/../../../lib" && pwd)"
...
echo "=== CONNECTOR PROOF: governed verify_income via AgentCore Identity outbound OAuth ==="

echo "-- 1. the system of record REALLY requires OAuth (no token / bad token are rejected) --"
NO_TOK="$(curl -s -o /dev/null -w '%{http_code}' "$SOR_URL?case_id=CASE-2026-0700")"
BAD_TOK="$(curl -s -o /dev/null -w '%{http_code}' -H 'Authorization: Bearer not-a-real-token' "$SOR_URL?case_id=CASE-2026-0700")"
if [ "$NO_TOK" = "401" ] && { [ "$BAD_TOK" = "401" ] || [ "$BAD_TOK" = "403" ]; }; then
  echo "  PASS | SOR rejects no-token ($NO_TOK) and bad-token ($BAD_TOK) — the dependency is genuinely OAuth-protected"; pass=$((pass+1))
else echo "  FAIL | SOR did not reject unauthenticated calls (no=$NO_TOK bad=$BAD_TOK)"; fail=$((fail+1)); fi

echo "-- 2. governed tool: caseworker calls verify_income; AgentCore Identity mints the outbound token --"
VI="$(call "$REV" "$TOOL_ID" '{"case_id":"CASE-2026-0700"}')"
if echo "$VI" | grep -q '"verified": *true' && echo "$VI" | grep -qi 'AgentCore Identity'; then
  echo "  PASS | verify_income returned authoritative income via the OAuth-protected SOR (token minted by Identity)"; pass=$((pass+1))
else echo "  FAIL | verify_income -> $VI"; fail=$((fail+1)); fi
...
OD="$(call "$OUT" "$TOOL_ID" '{"case_id":"CASE-2026-0700"}')"
if echo "$OD" | grep -qiE 'denied|not allowed|policy enforcement'; then echo "  PASS | outsider call to verify_income DENIED (Cedar deny-by-default, no new policy needed)"; pass=$((pass+1)); else echo "  FAIL | outsider not denied -> $OD"; fail=$((fail+1)); fi
