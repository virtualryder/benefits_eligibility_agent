...
**Validated release: [`v0.7.0-pilot-rc1`](https://github.com/virtualryder/benefits_eligibility_agent/releases/tag/v0.7.0-pilot-rc1)** (2026-09-09; **full-portfolio gate 17/17 from zero** on env `ben-fp2`, adding a preflight and a teardown assertion for AgentCore residue to the 15 checks of `v0.6.0-pilot-rc1` — 111 consolidated, kill switch, per-tenant budget, #168 lineage with zero orphans, runtime on the IaC role, e2e, teardown to zero residue; supersedes `v0.5.2-pilot-rc1`, which was offline-green and Tier-1 live but never carried a green full-portfolio gate)
...
| Claim | Status |
|---|---|
| Offline suite | **514 tests** (all pass locally except one CI-only gate that is skipped offline) (control-plane + 45 CDK stack-synthesis assertions) — authoritative count: [`RELEASE-MANIFEST.md`](RELEASE-MANIFEST.md) |
...
| Live governance demo (shell engine, legacy) | 29 checks against a deployed system in Cedar ENFORCE — a **separate** artifact from the 514 offline tests |
...
Suite: **514 offline tests** (the older `v0.1.2-pilot-rc1` tag predates the dependency migration and
