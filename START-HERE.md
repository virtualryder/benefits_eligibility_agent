...
| Claim | Status |
|---|---|
| Offline suite | **413 tests** (all pass locally except one CI-only gate that is skipped offline) (control-plane + 45 CDK stack-synthesis assertions) — authoritative count: [`RELEASE-MANIFEST.md`](RELEASE-MANIFEST.md) |
...
| Live governance demo (shell engine, legacy) | 29 checks against a deployed system in Cedar ENFORCE — a **separate** artifact from the 413 offline tests |
...
Suite: **413 offline tests** (the older `v0.1.2-pilot-rc1` tag predates the dependency migration and
