"""Exactly-once finalization â€” the commit gate that prevents committing the same benefits determination twice.
...
What is actually being asserted: a second finalize of the same case â€” a retried Lambda, a replayed
Step Functions execution, or a second approval path â€” must NOT write a second COMMITTED record, and
...
def test_replayed_finalize_writes_no_second_committed_record(finalize):
    ...
    assert second["submission_id"] == first["submission_id"], \
        "replay returned a DIFFERENT submission id â€” that is a second submission"
    assert len(committed) == 1, (
        "a second COMMITTED evidence record was written â€” this is the double-reporting defect "
