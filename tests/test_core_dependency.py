...
was green. For a benefits determination workflow that gap is a due-process risk: a retried or
replayed finalize writes a second COMMITTED record, i.e. the same adverse action — a denial, a
reduction, a termination of assistance — committed twice against the same person, producing two
notice clocks and two appeal windows the agency cannot reconcile.
...
DOMAIN_OVERRIDES = {
    "mask_pii": "Benefits masks the applicant identifiers a public-assistance case actually carries "
                "(name, SSN, address, DOB, phone, bank/routing for direct deposit) and mints the "
                "signed sanitized_ref over that entity set.",
    "provenance": "Benefits has NO external authoritative source to bind to — the eligibility engine "
                  "runs on HHS Federal Poverty Guidelines baked in as pinned configuration, so this "
                  "signer is single-domain and carries no GA-2 deid/lookup key split.",
}
...
def test_exactly_once_finalization_is_present_in_the_resolved_core():
    ...
    assert hasattr(finalize_signoff, "_exactly_once_marker"), (
        "the resolved finalize_signoff has no _exactly_once_marker — this is the double-commit "
        "gap re-opening, i.e. the same adverse action recorded twice against one person")
