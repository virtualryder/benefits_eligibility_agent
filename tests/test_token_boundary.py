...
Proves: (a) no tool schema in the manifest declares a credential field; (b) model-supplied credential-
shaped args are scrubbed from EVERY tool call and the runtime-held token is injected out-of-band into the
sign-off call only; (c) the sign-off audit payload and tool result never contain the bearer token. Pure
logic, no AWS. Wiring `wrap_mcp_client` into the live agent loop is the runtime concern."""
import json
import pathlib
import sys
...
MANIFEST = (ROOT / "agents" / "benefits-eligibility" / "manifest.yaml").read_text(encoding="utf-8")
...
def test_inject_only_for_signoff_and_overrides_model_token():
    a = token_boundary.prepare_args("assess-eligibility___assess_eligibility",
                                    {"household_size": 4, "access_token": "stolen"}, TOKEN)
    assert "access_token" not in a
    a = token_boundary.prepare_args("request-signoff___request_signoff",
                                    {"icsr_id": "CASE-1", "access_token": "stolen"}, TOKEN)
    assert a["access_token"] == TOKEN and a["icsr_id"] == "CASE-1"
...
def test_wrap_intercepts_positional_and_keyword_calls():
    ...
    c.call_tool_sync("t2", name="ben-core___draft_notice", arguments={"case": "x", "token": "stolen"})
...
def test_signoff_audit_payload_contains_no_token(monkeypatch):
    ...
    out = rs.handler({"icsr_id": "CASE-1", "access_token": TOKEN}, _Ctx())
