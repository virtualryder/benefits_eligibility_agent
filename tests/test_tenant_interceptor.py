...
def _event(method="tools/call", args=None, token=None, rid=7):
    ...
    if method == "tools/call":
        body["params"] = {"name": "ben-core___draft_notice", "arguments": dict(args or {})}
...
def test_tools_list_and_silo_pass_through_unchanged():
    ...
    # longer assert the Cedar context fields — a caller-supplied budget_ok is STRIPPED and the
    # server-authoritative value (True, since we are past the hard-breach gateway deny) is injected;
    # within_service_window is not asserted unless SERVICE_WINDOW_* is configured.
...
def test_verify_rejects_bad_sig_and_bind_sets_routing_context(monkeypatch):
    ...
    try:
        ...
        assert tenancy.route_store("ben-e2e-case-store", "case-store") == "ben-e2e-pha-alameda-case-store"
