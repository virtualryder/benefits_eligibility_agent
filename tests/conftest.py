...
agent-specific module (mask_pii, provenance, workflow_guards, sanitized, case_store, ingest_case,
tenancy) shadows the packaged one if a name ever collides. `tests/test_core_dependency.py` asserts
that every such shadow is a DECLARED domain override, because a silent shadow would reintroduce
...
os.environ.setdefault("PROVENANCE_SECRET", "ben-unit-provenance-secret")
...
for _p in (ROOT / "lib" / "controls", ROOT / "agents" / "benefits-eligibility" / "tools", ROOT / "tests"):
