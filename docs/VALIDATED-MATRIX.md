...
Release `v0.7.0-pilot-rc1` (2026-09-09; full-portfolio gate 17/17 from zero — the single source of truth for the current tag is the repo-root `RELEASE` file. Predecessors: `v0.6.0-pilot-rc1` 2026-09-07, `v0.5.2-pilot-rc1` 2026-09-06 Tier-1 hardening + enforcement perimeter, `v0.4.0-pilot-rc1` 2026-09-03 kill switch + budget, `v0.3.0-pilot-rc1` 2026-09-02) · EP1 Gate-B run 2026-07-27 on `v0.1.2-pilot-rc1` (env `ben-val1`,
us-east-1) · AgentCore multi-tenant / transparency runs 2026-09-02 (`ben-e2e`, `ben-mt`, `ben-mt2`, `ben-mt3`).
Latest from-zero live gate: **`ben-fpg`, 2026-09-10, 21 of 21, zero residue** — `evidence/FULL-PORTFOLIO-GATE-benefits_runtime_agent.json`. It is the first run on which `CONN_deploy` required the outbound leg to have been exercised (`outbound_ok=True`) rather than the connector's artifacts merely existing; two earlier runs passed the old rule over a system of record that trusted nothing.

Current offline suite: **514 tests** on main (authoritative count: `RELEASE-MANIFEST.md`); `v0.3.0-pilot-rc1` stood at 154 <!-- count-gate:historical -->.
