# MCP Secure Gateway & Portability (Benefits)
...
The agent never calls a tool Lambda directly. Every governed tool (intake, the eligibility engine lookup, mask,
eligibility, duplicate, adverse-action determination-prepare, draft, sign-off) is an **MCP target** behind a gateway that
...
- **Authorization:** Cedar deny-by-default — `pv_reviewer_permit`, `mask_before_{assess,adverse-action determination,draft}`,
