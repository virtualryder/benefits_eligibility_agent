...
- **Append-only, tamper-evident WORM audit:** an append-only, tamper-evident record (append-only DynamoDB + S3 Object Lock) capturing `INTENT → COMMITTED` for the due-process and IRS Pub 1075 evidence trail.
...
6. **Manifest + validate** — the whole agent is one manifest; deploy; end-to-end run (Cedar allow/deny, masking, eligibility, real Bedrock notice, tamper-evident audit) + negative tests; teardown.
...
- **IRS Publication 1075** (safeguarding Federal Tax Information used in income verification) → fail-closed masking + least-privilege Cedar + tamper-evident audit + encryption.
