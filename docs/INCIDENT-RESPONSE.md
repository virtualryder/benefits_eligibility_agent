# Incident Response — Benefits determination Assistant

*EDU-parallel IR procedure for the pilot, adapted to drug-safety. PII is HIPAA-covered; the platform
supports regulated safety reporting (ICH system-of-record / 7 CFR 273 / 42 CFR 435 / program-integrity) and, where applicable, 21 CFR Part
...
| Role | Responsibility |
|---|---|
| **Incident lead** | PV/benefits-program SME office lead — owns the incident, decides notification |
...
| **benefits-program SME / safety physician** | Assesses any impact on regulatory reporting obligations/timelines |
...
- **Guard-failure metric** (`Benefits/Governance :: GuardFailed`) — forged/tampered evidence.
- **WORM audit ledger** — tamper-evident hash chain (`verify_chain`).
- **CloudWatch alarms** on the above (ObservabilityStack).

## Runbooks

### R1 — Suspected PII in telemetry / stores
...
### R2 — A wrong eligibility/narrative reached a downstream process
Retract/correct; the assistant's outputs are drafts/estimates gated by a human, so trace the approval;
the **benefits-program SME assesses any impact on expedited-reporting timelines** (a mis-assessed serious/unexpected case
is a processing-clock risk); root-cause (stale thresholds, an unverified background, a duplicate that
...
notification per the institution's procedure and timelines; the benefits-program SME separately assesses whether the
...
benefits-program SME's determination under the applicable rule and facts.

## Before real (PII) data: tabletop

Run a tabletop of R1 and R2 with the PV/benefits-program SME office, IT/security, and privacy officer before the
