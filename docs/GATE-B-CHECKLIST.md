...
| **B6 · Load / replay** — concurrency + exactly-once replay storm | (harness / offline) | ✅ (255 offline tests) | ☐ prod-scale live load (customer-side Gate-B exit) |
...
- Signed `sanitized_ref` masking proof; token boundary; deterministic guard set with AdverseNoticeHold;
  ...
  ENFORCE with the benefits tool/policy set — all asserted by the **255-test suite** (incl. 27 CDK assertions).
