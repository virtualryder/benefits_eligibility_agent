...
git checkout "$(cat RELEASE)"          # the released tag, never main — the harness enforces this
                                       # RELEASE is the single source of truth for which tag that is;
                                       # today it reads v0.7.0-pilot-rc1
...
|---|---|---|
| `preflight.on_release_tag` | you are on the tag named in `RELEASE` (today `v0.7.0-pilot-rc1`) with a clean tree | you verified the *released* artifact, not a working copy |
