/* Customer deck — Governed AI for Benefits Eligibility (accelerator on AWS AgentCore) */
const pptxgen = require("pptxgenjs");
const p = new pptxgen();
p.layout = "LAYOUT_WIDE";
p.author = "Benefits AgentCore Accelerator";
p.title = "Governed AI for Benefits Eligibility";
...
function footer(s, n) {
  s.addText("Governed AI for Benefits Eligibility · on Amazon Bedrock AgentCore", { x: 0.6, y: 7.06, w: 9, h: 0.3, color: MUTED, fontSize: 8, fontFace: BF, margin: 0 });
...
(() => {
  ...
  s.addText("PUBLIC BENEFITS  ·  GOVERNED AI ON AWS", { x: 0.7, y: 1.4, w: 11, h: 0.4, fontFace: BF, fontSize: 13, bold: true, color: MINT, charSpacing: 2, margin: 0 });
  s.addText("Governed AI for\nBenefits Eligibility", { x: 0.66, y: 2.0, w: 11.2, h: 2.0, fontFace: TF, fontSize: 42, bold: true, color: WHITE, lineSpacingMultiple: 1.02, margin: 0 });
  s.addText("An accelerator that intakes, de-identifies, screens, and drafts eligibility determinations under strict governance — fail-closed on PII, tamper-proof by design, and never able to adjudicate on its own. Built on Amazon Bedrock AgentCore, running in your AWS account.",
    { x: 0.7, y: 4.15, w: 10.2, h: 1.3, fontFace: BF, fontSize: 15.5, color: ICE, lineSpacingMultiple: 1.18, margin: 0 });
  const chips = ["Compliant by construction", "Runs in your account", "Caseworker-in-command"];
  let cx = 0.7;
  chips.forEach((c) => {
  ...
  s.addNotes("Frame for an eligibility-program, privacy, and compliance audience. The promise is governance first: this agent helps caseworkers without ever stepping outside the controls due process and StateRAMP expect.");
})();
...
(() => {
  ...
  title(s, "Caseloads rise. Every intake step must stay compliant.");
  s.addText("Applications keep coming, intake is largely manual, and the processing clocks don't wait — SNAP expedited service is 7 days. AI could take the assembly load off caseworkers — but only if it operates entirely inside your compliance envelope.",
    { x: 0.62, y: 1.5, w: 12.0, h: 0.85, fontFace: BF, fontSize: 15, color: INK, lineSpacingMultiple: 1.15, margin: 0 });
  const items = [
    ["Volume & clocks", "Rising application counts against fixed expedited (7-day) and standard (30-day) deadlines.", TEAL],
    ["Manual assembly", "Skilled caseworkers spend time on extraction, screening, and notice drafting — not judgment.", AMBER],
    ["Zero compliance slack", "PII handling, due-process notice, and the caseworker determination leave no room for an ungoverned tool.", MINT],
  ];
  ...
  footer(s, 2);
  s.addNotes("Meet the agency where they are: caseload pressure, manual toil, and a compliance bar that rules out ungoverned AI. The rest of the deck resolves that tension.");
})();
...
(() => {
  ...
  s.addText("The agent intakes the application, de-identifies it, runs a transparent eligibility check, drafts the determination notice, and prepares it for review. Then it stops. A qualified caseworker makes the determination and commits — the agent cannot.",
    { x: 0.62, y: 1.95, w: 11.8, h: 0.9, fontFace: BF, fontSize: 15, color: ICE, lineSpacingMultiple: 1.16, margin: 0 });
  const cols = [
    ["The agent DOES", MINT, ["Intake & extract the decision fields", "De-identify the application (fail-closed)", "Run the eligibility rules engine", "Draft the determination notice", "Record a tamper-proof audit + request sign-off"]],
    ["The agent NEVER", "C0392B", ["See un-masked PII without controls", "Assess or draft on un-de-identified data", "Approve its own work", "Commit the determination", "Delete or alter the audit trail"]],
  ];
  cols.forEach((c, ci) => {
  ...
  footer(s, 3);
  s.addNotes("The 'does vs never' framing earns trust with privacy and program-integrity teams. The 'never' column is enforced by policy, not by prompt — that's the difference.");
})();
...
(() => {
  ...
  title(s, "Agent-assisted intake, caseworker-in-command");
  const steps = [["Intake", "the application", TEAL, "AGENT"], ["De-identify", "the PII (fail-closed)", TEAL, "AGENT"], ["Assess & draft", "eligibility + notice", TEAL, "AGENT"], ["Review &", "sign off", AMBER, "CASEWORKER"], ["Commit", "the determination", NAVY, "AFTER APPROVAL"]];
  ...
  s.addText([
    { text: "Your caseworkers stay in command — ", options: { bold: true, color: TEAL } },
  ...
  footer(s, 4);
  s.addNotes("Blue = agent-assisted, amber = the caseworker, navy = the gated commit. Emphasize: judgment stays human; toil goes to the agent.");
})();
...
(() => {
  ...
  const rows = [
    ["Fail-closed PII de-identification", "The application is de-identified before the model or the audit sees it. No masking → no assessment, no draft.", "IRS Pub 1075"],
    ["Immutable audit trail", "Append-only, write-once evidence of every decision and state change — the writer can't alter it.", "Fair hearing"],
    ["Human sign-off (separation of duties)", "A different qualified caseworker approves with a bound, single-use token before any determination is committed.", "Due process"],
    ["Deny-by-default authorization", "Every tool call is checked against policy; nothing runs unless explicitly permitted.", "NIST 800-53 AC"],
    ["Verified identity", "Actions are bound to an authenticated caseworker via your identity provider.", "StateRAMP · IA"],
  ];
  ...
  footer(s, 5);
  s.addNotes("This is the slide your compliance and privacy stakeholders came for: each control mapped to the requirement it satisfies. Walk it slowly.");
})();
...
(() => {
  ...
  const items = [
    ...
    ["Your data stays yours", "PII is masked before any model call; nothing leaves your boundary for third-party processing."],
    ["Your identity provider", "Federate your existing IdP; actions are bound to your caseworkers and your roles."],
  ...
  footer(s, 6);
  s.addNotes("The trust posture in one slide: your account, your data, your IdP, reversible. This addresses the security review that inevitably follows — and aligns with StateRAMP expectations.");
})();
...
(() => {
  ...
  stats.forEach((st, i) => {
    const x = x0 + i * (sw + gap);
    card(s, x, y, sw, 1.35);
    s.addText(st[0], { x, y: y + 0.12, w: sw, h: 0.7, align: "center", fontFace: TF, fontSize: 32, bold: true, color: st[2], margin: 0 });
  ...
  const proofs = [
    ...
    "A different caseworker required to approve; approvals are single-use",
  ...
  footer(s, 7);
  s.addNotes("Concrete, honest proof points. 'Not a slideware demo' — it's reproducible and enforced. The eligibility engine is deterministic and transparent, so a caseworker can defend each determination at a hearing.");
})();
...
(() => {
  ...
  ["The governed agent + policies", "The six intake tools", "Fail-closed PII masking", "The deterministic eligibility rules engine", "The human sign-off workflow + audit", "Infrastructure-as-code + documentation"].forEach((t, i) => {
    const y = 2.5 + i * 0.55;
  ...
  ["Your identity provider + caseworker roles", "Connection to your benefits system of record", "The authoritative program rules (legal review)", "Notice language, appeal rights, fair hearing", "Validation + StateRAMP authorization to operate"].forEach((t, i) => {
    const y = 2.5 + i * 0.64;
  ...
  s.addText("An accelerator, not a finished product — the program rules, validation, and certification for your intended use stay with you.", { x: 0.62, y: 6.2, w: 12.1, h: 0.55, align: "center", valign: "middle", fontFace: BF, fontSize: 13, bold: true, italic: true, color: WHITE, margin: 0 });
  footer(s, 8);
  s.addNotes("Honesty builds trust. Be explicit that the authoritative rules (and their legal review), connectors to their real systems, validation, and StateRAMP ATO stay with them. The shipped thresholds are illustrative.");
})();
...
(() => {
  ...
  title(s, "Caseworkers spend their time on judgment, not assembly");
  const items = [
    ["Faster first drafts", "The agent produces a de-identified determination and a drafted notice for review in minutes — caseworkers refine instead of starting from a blank page.", TEAL],
    ["More consistent intake", "Every application runs the same governed path — the same masking, the same rules engine, the same audit — reducing variability and disparate treatment.", MINT],
    ["Audit-ready by construction", "Evidence is captured automatically as the work happens, ready for a fair hearing or a StateRAMP assessment — not reconstructed later.", AMBER],
    ["Caseworkers stay in command", "The determination and the commit remain human — the agent never makes the call.", NAVY],
  ];
  ...
  footer(s, 9);
  s.addNotes("Value framed honestly as assist, not replacement. Emphasize consistency (which reduces disparate-treatment risk), audit-readiness for hearings, and keeping caseworkers in command.");
})();
...
(() => {
  ...
  s.addText("Governed AI you can actually put in front of an auditor — and a hearing officer. Let's prove it on your data.", { x: 0.62, y: 5.75, w: 12, h: 0.5, fontFace: TF, fontSize: 16, italic: true, bold: true, color: MINT, margin: 0 });
