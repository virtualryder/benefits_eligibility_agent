...
def build_canary_case(marker):
    """A synthetic benefits application carrying the marker as name, SSN-shaped id (900- reserved
    range), and address — the highest-risk PII shapes — plus the non-PII decision fields."""
    return {
        "case_id": f"CANARY-{marker[-19:-14]}",
        "application": (f"Applicant {marker} (SSN 900-00-{marker[7:11]}) residing at 1 {marker} Street, "
                        f"Los Angeles CA 90001. Household size 3. Monthly income $1200. "
                        f"Liquid resources $50."),
        "canary": True,
    }
...
def main():
    ...
    if args.execute and not args.sweep_only:
        ...
        ing = json.loads(lam.invoke(FunctionName=f"{args.prefix}-ingest-application",
                                    Payload=json.dumps({"application": case["application"], "case_id": case["case_id"]}).encode())["Payload"].read())
