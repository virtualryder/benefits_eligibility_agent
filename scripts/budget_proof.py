...
def model_log_sums(logs, group, tenant, start_ms, end_ms, session_id=None):
    ...
    session) AND the drafter's server-side Converse (requestMetadata.tenant + component=draft_notice) -
...
def main():
    ...
    a = ap.parse_args()
    prefix, region = f"ben-{a.env}", a.region
    ...
    if ing.get("case_ref"):
        ex = sfn.start_execution(stateMachineArn=wf["ControllerArn"], name="bgproof-" + case_b.lower(),
                                 input=json.dumps({"case_id": case_b, "requester": "bg-cw-b", "case_ref": ing["case_ref"],
                                                   "redetermination": {"change_type": "NEW"}, **ing.get("tenant_binding", {})}))["executionArn"]
        for _ in range(36):
            time.sleep(5)
