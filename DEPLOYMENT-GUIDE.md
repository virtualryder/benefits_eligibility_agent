...
```bash
git checkout v0.5.2-pilot-rc1                 # release tag (offline-green; live re-gated 2026-09-06 on ben-t1), never main
...
```bash
python -m pytest tests/ -q                    # 298 pass locally (+1 CI-only gate = 328 tests): control-plane + CDK synthesis + pass-by-ref + canary + doc-integrity gates
python -m pytest tests/test_cdk_stacks.py -q  # 45 CDK assertions (synthesizes all 7 stacks + the multi-tenant variants)
```
