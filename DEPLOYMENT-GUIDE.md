...
```bash
git checkout v0.7.0-pilot-rc1                 # release tag (full-portfolio gate 17/17 from zero, 2026-09-09), never main
...
```bash
python -m pytest tests/ -q                    # 298 pass locally (+1 CI-only gate = 513 tests): control-plane + CDK synthesis + pass-by-ref + canary + doc-integrity gates
