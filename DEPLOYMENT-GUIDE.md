...
```bash
git checkout v0.6.0-pilot-rc1                 # release tag (full-portfolio gate 15/15 from zero, 2026-09-07), never main
...
```bash
python -m pytest tests/ -q                    # 298 pass locally (+1 CI-only gate = 396 tests): control-plane + CDK synthesis + pass-by-ref + canary + doc-integrity gates
