...
```bash
python -m pytest tests/ -q                    # 298 pass locally (+1 CI-only gate = 299 tests): control-plane + CDK synthesis + pass-by-ref + canary + doc-integrity gates
python -m pytest tests/test_cdk_stacks.py -q  # 43 CDK assertions (synthesizes all 7 stacks + the multi-tenant variants)
```
