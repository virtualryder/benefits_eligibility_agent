...
```bash
python -m pytest tests/ -q                    # 295 pass locally (+1 CI-only gate = 296 tests): control-plane + CDK synthesis + pass-by-ref + canary + doc-integrity gates
python -m pytest tests/test_cdk_stacks.py -q  # 40 CDK assertions (synthesizes all 7 stacks + the multi-tenant variants)
```
