...
| **Pilot tag** | `v0.3.0-pilot-rc1` (RELEASE file) — **cut 2026-09-02 from the tree validated by the AgentCore from-zero, hybrid multi-tenant, per-tenant audit routing and full-transparency runs**; `v0.1.2-pilot-rc1` is the EP1 Gate-B tag (2026-07-27) |
| **Offline test suite** | **173** passing **on current main** (control-plane + **18 CDK stack-synthesis** assertions + doc-integrity, independent-verification, CI-completeness and doc-count gates). 172 run locally; 1 CI-completeness gate runs only in CI (`skipif` outside CI). Tag `v0.3.0-pilot-rc1` stood at 154 <!-- count-gate:historical -->; main has grown since with the kill switch (task 127) and the per-tenant budget (task 128) — the next tag will carry them. The older `v0.1.2-pilot-rc1` tag predates the governed-core dependency migration and stood at 101. |
...
| **Per-tenant token + USD budget** | **PASS (2026-09-03, `ben-mt6`, 23/23)** — meter == model-invocation log; cap refusals at runtime (mid-session) / gateway / drafter; 60/85/100 % alarms; AWS Budgets USD ceiling with IAM-deny action + breach → kill switch; 0-unexpected-errors sweep — `evidence/AGENTCORE-BUDGET-2026-09-03.md` |
...
- **173 offline tests** — the CI suite (control-plane + 18 CDK synthesis + 3 doc-integrity gates + 4 independent-verification gates + 3 CI-completeness gates + the doc-count gate). Authoritative offline number. Locally you see `172 passed, 1 skipped`: one gate asserts the CDK libs are installed and only runs inside CI.
