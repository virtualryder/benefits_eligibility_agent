...
def handler(event, context):
    ...
    # through Step Functions state). When input arrived by ref, the masked text is NOT echoed back —
    # downstream consumers load it server-side from the sanitized-artifact store via the signed ref.
    by_ref = False
    ...
    out = {"deidentified": True, "entities_masked": len(ents),
           ...
           "note": ("pass sanitized_ref (JSON) to assess_eligibility/redetermine/overpayment/draft_notice — "
