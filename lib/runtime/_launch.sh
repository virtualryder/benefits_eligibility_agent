...
# Platform guardrail on the runtime's own model calls (third review): from the compute stack outputs, so the
# runtime role's mandatory-guardrail IAM condition is satisfied on every call. Empty = no guardrail deployed.
