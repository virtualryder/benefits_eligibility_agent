...
> release tag [`v0.7.0-pilot-rc1`](https://github.com/virtualryder/benefits_eligibility_agent/releases/tag/v0.7.0-pilot-rc1)**
...
```bash
python -m pip install --require-hashes -r requirements-core.txt   # governed-core 1.11.1, sha256-pinned
...
## Supply chain & CI security gates (2026-09-08, SEC-1)

`.github/workflows/security.yml` runs **five BLOCKING gates** on every push, every PR and weekly.
Nothing here is report-only.

| Gate | What it does |
|---|---|
| **bandit** | Python static analysis, medium+ severity and confidence, hard fail |
| **detect-secrets** | Against the reviewed `.secrets.baseline`; any finding not in it fails the build |
| **checkov** | Over the **synthesized** CloudFormation, against a committed baseline |
| **CodeQL** | `security-extended` |
| **trivy** | The AgentCore Runtime image actually built from `lib/runtime`, HIGH and CRITICAL fail |

**Read the history, not just the table.** When these were first made to run properly, four of them
had never actually executed: the secrets baseline was UTF-16 and unreadable by the scanner, the
secret-diff compared a timestamp so it could never pass, the checkov job synthesized zero templates
because the pinned core was not installed, and trivy was pinned to an action tag that does not
exist. The runtime base image was 30 fixable HIGH/CRITICAL behind, hidden behind the scan that was
not running. `docs/GAP-CLOSURE-BACKLOG.md` in the platform repo records each (L44–L47, L50).

Regenerating the secrets baseline has a procedure, and it matters: see
[`docs/SECRETS-BASELINE.md`](docs/SECRETS-BASELINE.md). A baseline generated by the wrong scanner
version, or from a CRLF working tree, disagrees with the runner on findings that are not new.

## Policy provenance in every audit record (#232)

Every audit row carries `policy_version`, `rule_version` and `deployment_version` inside its hashed
logical record. Until 2026-09-08 the CDK set none of them, so every row a deployed Lambda wrote said
`"policy_version": "unset"` — in the WORM ledger, for a regulated workflow. They are now
content-addressed over the artefacts actually deployed:

    POLICY_VERSION      cedar-<sha12>      over policies/*.cedar
    RULE_VERSION        manifest-<sha12>   over agents/*/manifest.yaml
    DEPLOYMENT_VERSION  <the RELEASE tag>

Content-addressed, so it changes exactly when the thing it names changes. `tests/test_policy_provenance.py`
...
| Portfolio-level brief | There are **two** partner briefs and they are not the same document. This one is scoped to **this pack**. The portfolio-wide brief — four packs, the Cedar policy set, the gap backlog and the partner sequence — lives in the platform repo as `AEGIS-PARTNER-BRIEF-2026-09-10.docx`, with the architecture and the deck beside it. Send that one to a partner; send this one when the conversation is about the benefits pack specifically. |
